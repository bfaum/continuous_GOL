...
__global__ void update(const double* __restrict__ board, double* new_board, int width, int height) {
    ...
    if (x < width && y < height) {
        ...
        new_board[idx] = .55*val + 1.33*(val * a + (1 - val) * d);
    }
}
    ...
    if (x < board_width && y < board_height) {
        ...
        for (int i = 0; i < size; i++) {
            for (int j = 0; j < size; j++) {
                ...
                pixels[pixel_idx] = (uint8_t)fmin((true_color * 255 * 1 + (double)pixels[pixel_idx]*.0),255.0);       // R
                pixels[pixel_idx + 1] = (uint8_t)fmin((true_color * 255 * 1 + (double)pixels[pixel_idx + 1]*.99),255.0);   // G
                pixels[pixel_idx + 2] = (uint8_t)fmin((true_color * 255 * 1 + (double)pixels[pixel_idx + 2]*.9),255.0);   // B
            }
        }
    }
}
