...
__global__ void update(const double* __restrict__ board, double* new_board, int width, int height) {
    ...
    if (x < width && y < height) {
        ...
        new_board[idx] = .91*val + 1*(val * a + (1 - val) * d);
    }
}
    ...
    if (x < board_width && y < board_height) {
        ...
        for (int i = 0; i < size; i++) {
            for (int j = 0; j < size; j++) {
                ...
                pixels[pixel_idx] = (uint8_t)fmin((true_color * 255 * .04 + (double)pixels[pixel_idx]*.98),255.0);       // R
                pixels[pixel_idx + 1] = (uint8_t)fmin((true_color * 255 * .02 + (double)pixels[pixel_idx + 1]*.999),255.0);   // G
                pixels[pixel_idx + 2] = (uint8_t)fmin((true_color * 255 * .03 + (double)pixels[pixel_idx + 2]*.995),255.0);   // B
            }
        }
    }
}
...
while running:
    ...
    if framenum == 307:
        ...
        test_time_ms = (end_test_time - start_test_time) * 1000
        print(f"time: {test_time_ms:.2f} ms")
    pygame.time.delay(0)
