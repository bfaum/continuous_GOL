...
__global__ void update(const double* __restrict__ board, double* new_board, int width, int height) {
    ...
    if (x < width && y < height) {
        ...
        new_board[idx] = 0.91*val + 1*(val * a + (1 - val) * d);
    }
}
...
__global__ void fill_pixels(double* board, uint8_t* pixels, int board_width, int board_height, int size) {
    ...
    if (x < board_width && y < board_height) {
        ...
        uint8_t color = (uint8_t)(fmin(fmax(board[y * board_width + x], 0.0), 1.0) * 255);

        for (int i = 0; i < size; i++) {
            for (int j = 0; j < size; j++) {
                int pixel_idx = ((pixel_y_start + i) * board_width * size + (pixel_x_start + j)) * 3;
                pixels[pixel_idx] = color * 1;       // R
                pixels[pixel_idx + 1] = color * 1;   // G
                pixels[pixel_idx + 2] = color * 1;   // B
            }
        }
    }
}
...
class Board:
    def __init__(self, size):
        ...
        self.board = np.random.choice([0, 1], size=(self.height, self.width), p=[13/14, 1/14]).astype(np.double)
